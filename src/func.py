from src.abstract_func import Functions
import json

class ListVacancies(Functions):
    ...
    def add_vacancy(self, name_vac):
        ...
        with open("data/filtered_vacancies.json", "r", encoding="utf-8") as f:
            list = json.load(f)
        ...
        with open("data/filtered_vacancies.json", "w", encoding="utf-8") as f:
            f.write(list_vacancies_add)
        return list_vacancies_add

    def get_data(self, criterion):

        """Метод получения данных из файла по указанным требованиям"""
